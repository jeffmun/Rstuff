...
# `7MM"""YMM                             `7MM
#   MM    `7                               MM
#   MM   d    `7M'   `MF',p6"bo   .gP"Ya   MM
#   MMmmMM      `VA ,V' 6M'  OO  ,M'   Yb  MM
#   MM   Y  ,     XMX   8M       8M""""""  MM
#   MM     ,M   ,V' VA. YM.    , YM.    ,  MM
# .JMMmmmmMMM .AM.   .MA.YMbmd'   `Mbmmd'.JMML.
#
####################################################################################################

cat("\njm_excel version: 2018-11-26")
...
GetData <- function(file, usr, pwd, localpath)
{
  require(httr)
  
  myurl <- paste("https://uwac.autism.washington.edu//research//stats//DataFileHandler.ashx?file="
  , file, sep="")
  localfile <- paste( localpath, file, sep="")
  
  
  r <- GET(myurl 
    , authenticate(user = usr, password = pwd, type = "ntlm")
...
XLinfo <- function(xlfile, startrow=1, hasheader=TRUE, keepdata=TRUE, mergedata=FALSE
			,onlysheets=NULL, printflag=FALSE)
{ 	###################################################################################
    ...
    if(class(wb)!="Workbook" & class(wb)!="workbook" ) {
		...
		if(length(s) > 1)
		{
			...
			if(n_cols_LTmax > 0)
			{
				...
				cat("## WARNING: The following columns are used less than others.\n")
